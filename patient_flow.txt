...
<head>
    ...
    <title>PatientFlow – Strict Department Visibility & Control</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<div class="app-wrapper">
    ...
    <div class="app-header">
        ...
        <div class="patient-selector">
            <span style="color: white;">👥 ACTIVE PATIENT:</span>
            <select id="patientDropdown" onchange="switchActivePatient()"></select>
            <button class="btn btn-primary" onclick="showCreatePatientModal()">➕ NEW PATIENT</button>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="sidebar">
            ...
            <div style="margin-top: 40px; color: #b8d0e0; font-size: 14px; border-top: 1px solid #2a4560; padding-top: 24px;">
                <span style="display: block; margin-bottom: 16px;">📋 Active patient summary</span>
            ...
            <div style="margin-top: 30px; background: #17324b; border-radius: 18px; padding: 16px;">
                <span style="color: #ffd966;">🔒 STRICT POLICY:</span>
                <span style="display: block; margin-top: 10px; color: #c2d9f0; font-size: 13px;">
                    • Each department sees ONLY their own orders<br>
                    • ONLY the assigned department can change status<br>
                    • Doctor can view all orders but CANNOT modify
                </span>
            </div>
        </div>
        ...
        <div class="main">
                ...
                <div style="width: 100%;">
                    ...
                    <div class="patient-fields">
                        ...
                        <div class="field-group" style="justify-content: flex-end;">
                            <button class="btn" style="background: #1e5a9b; color: white; margin-top: 16px;" onclick="updateCurrentPatientInfo()">💾 UPDATE PATIENT</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card action-card">
                <h3 style="display: flex; gap: 8px;">➕ CREATE ORDER — for <span id="activePatientNameInOrder" style="background: #d1e6ff; padding: 4px 16px; border-radius: 60px;">John Doe</span></h3>
                ...
                <div class="datetime-picker">
                    <span style="font-weight: 600; color: #0f456b;">⏱️ ORDER TIMESTAMP:</span>
                    <input type="datetime-local" id="customOrderTime" value="2026-02-12T14:30">
                    <button class="btn-primary" style="background: #1e6eb5; color: white; padding: 12px 28px;" onclick="addActionForCurrentPatient()">✓ PLACE ORDER</button>
                </div>
                <p style="font-size: 13px; margin-top: 12px; color: #256ba0;">ⓘ Order will be assigned to the responsible department. Only that department can see and update it.</p>
            </div>

            <!-- TIMELINE CARD - STRICT VISIBILITY & CONTROL -->
            <div class="card">
                <h3>📋 CARE TIMELINE · <span id="timelinePatientName">John Doe</span>
                ...
                <div id="doctorViewNotice" class="doctor-view-notice" style="display: none;">
                    👁️ <strong>Doctor View:</strong> You can see all orders for this patient, but you cannot modify status. Only the assigned department can update orders.
                </div>
                
                <div id="timeline"></div>
                <div id="emptyTimelineMsg" class="empty-state" style="display: none;">📭 No orders visible for this role.</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    // -------------------------------------------------------------
    // PATIENTFLOW — STRICT DEPARTMENT VISIBILITY & CONTROL
    // -------------------------------------------------------------
    // RULES:
    ...
    function refreshAllPatientUI() {
        ...
        document.getElementById('sidebarPatientSummary').innerHTML = `
            ...
            <span style="display: block; margin-top: 8px; color: #b8d6ed;">Age: ${patient.age || '—'}</span>
            <span style="display: block; color: #b8d6ed;">Diagnosis: ${patient.diagnosis || '—'}</span>
            <span style="display: block; margin-top: 12px; background: #0e3e5c; padding: 6px 12px; border-radius: 30px;">🆔 ${patient.id}</span>
        `;
    ...
    function showCreatePatientModal() {
        ...
        refreshAllPatientUI();
        renderView();
        alert(`✅ New patient ${newName} (${newId}) created.`);
    }
    ...
    function renderView() {
        ...
        if (filteredOrders.length === 0) {
            ...
            filteredOrders.sort((a,b) => a.id - b.id).forEach(order => {
                ...
                const deptBadge = `<span class="dept-tag">🏥 ${order.department}</span>`;
                ...
                div.innerHTML = `
                    ...
                    <div style="display: flex; gap: 18px; align-items: center; flex-wrap: wrap;">
                        ${deptBadge}
                        <span style="color: #2d6190; background: #eef5fc; padding: 4px 16px; border-radius: 40px;">🕒 ${order.timestamp}</span>
                    </div>
                `;
                ...
                if (order.status !== 'Completed' && role === order.department) {
                    ...
                    if (order.status !== 'In Progress') {
                        ...
                        progressBtn.innerText = '⏳ Mark In Progress';
